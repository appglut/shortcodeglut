/**
 * Shortcodeglut Timeline Add to Cart Handler
 * Handles "Add to Cart" -> "View Cart" button functionality for timeline shortcode
 */
...
(function($) {
    ...
    $(document.body).on('click', '.shortcodeglut-timeline-wrapper .shortcodeglut-add-to-cart-btn', function(e) {
        var $button = $(this);
        ...
        if (!processedButtons[buttonId]) {
            ...
            if ($button.hasClass('ajax_add_to_cart')) {
                ...
                if (typeof wc_add_to_cart_params !== 'undefined' && wc_add_to_cart_params.wc_ajax_url) {
                    ...
                    ajaxUrl = window.shortcodeglutTimeline ? window.shortcodeglutTimeline.ajax_url : '/wp-admin/admin-ajax.php';
                }
                ...
                $.ajax({
                    ...
                    beforeSend: function(xhr) {
                        if (ajaxUrl.indexOf('admin-ajax.php') !== -1 && window.shortcodeglutTimeline) {
                            xhr.setRequestHeader('X-WP-Nonce', window.shortcodeglutTimeline.nonce);
                        }
                    },
    ...
    $(document.body).on('added_to_cart', function(e, fragments, cart_hash, $thisButton) {
        if ($thisButton && $thisButton.hasClass('shortcodeglut-add-to-cart-btn') &&
            $thisButton.closest('.shortcodeglut-timeline-wrapper').length) {
    ...
    function hideWooCommerceLoaders() {
        $('.shortcodeglut-timeline-wrapper .blockUI, .shortcodeglut-timeline-wrapper .blockOverlay, .shortcodeglut-timeline-wrapper .woocommerce-loader').hide();
    }
    ...
    $(document).ajaxSuccess(function(event, xhr, settings) {
        if (settings.url && settings.url.indexOf('wc-ajax') !== -1) {
            $('.shortcodeglut-timeline-wrapper .shortcodeglut-view-cart').parent().find('.added_to_cart').remove();
        }
    });
