...
class SaleProductsShortcode {
	...
	public function register_assets() {
		...
		wp_register_style(
			'shopglut-sale-products-templates',
			false,
			array(),
	...
	private function render_with_template( $product, $template ) {
		...
		wp_enqueue_style( 'shopglut-sale-products-templates' );
		if ( ! empty( $css ) ) {
			// Use a unique handle for each template's inline CSS
			$inline_handle = 'shopglut-template-' . $template_instance_id;
			wp_register_style( $inline_handle, false, array(), SHORTCODEGLUT_VERSION );
