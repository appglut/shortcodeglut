/**
   * Shortcodeglut Table List Add to Cart Handler
   * Handles "Add to Cart" -> "View Cart" button functionality for table list shortcode
   */

  (function($) {
      'use strict';

      // Store processed buttons to prevent duplicate transformations
      var processedButtons = {};

      // Handler for add to cart buttons in table list                                                                                                   
       $(document.body).on('click', '.shortcodeglut-table-list-cart .shortcodeglut-add-to-cart-btn', function(e) {
          const $button = $(this);
          const buttonId = $button.attr('data-product_id') || Math.random().toString(36);
          const cartUrl = $button.attr('data-cart-url');
          const productId = $button.attr('data-product_id'); // Must use underscore

          // Check if this is already a "View Cart" button
          if ($button.hasClass('shortcodeglut-view-cart')) {
              e.preventDefault();
              window.open(cartUrl, '_blank');
              return false;
          }

          // Check if button is already in loading state                                                                                                
          if ($button.hasClass('shortcodeglut-add-to-cart-loading')) {
              e.preventDefault();                                                                                                                       
              return false;
          }

          // Prevent default behavior for AJAX add to cart
          if ($button.hasClass('ajax_add_to_cart')) {
              e.preventDefault();
          }

          // Mark this button as being processed
          if (!processedButtons[buttonId]) {
              processedButtons[buttonId] = true;

              // Show loading state
              showLoadingState($button);

              // For AJAX add to cart
                if ($button.hasClass('ajax_add_to_cart')) {
                  const productId = $button.attr('data-product_id'); // Use underscore
                  const quantity = $button.attr('data-quantity') || 1;                                                                                    
   
                 var ajaxUrl = '';
            if (typeof wc_add_to_cart_params !== 'undefined' && wc_add_to_cart_params.wc_ajax_url) {
                ajaxUrl = wc_add_to_cart_params.wc_ajax_url.toString().replace('%%endpoint%%', 'add_to_cart');
            } else if (typeof woocommerce_params !== 'undefined' && woocommerce_params.ajax_url) {
                ajaxUrl = woocommerce_params.ajax_url;
            } else {                                                                                                                                              
                // Fallback to WordPress admin AJAX
                ajaxUrl = window.shortcodeglutTableAjax ? window.shortcodeglutTableAjax.ajax_url : '/wp-admin/admin-ajax.php';                              
            }               

            // Add debug logging
            console.log('Shortcodeglut: Adding product via AJAX', {
                productId: productId,
                quantity: quantity,
                ajaxUrl: ajaxUrl
            });

                 $.ajax({
                type: 'POST',
                url: ajaxUrl,
                data: {
                    product_id: productId,
                    quantity: quantity
                },
      beforeSend: function(xhr) {
          // Add nonce if using admin-ajax.php
          if (ajaxUrl.indexOf('admin-ajax.php') !== -1 && window.shortcodeglutTableAjax) {                                                           
              xhr.setRequestHeader('X-WP-Nonce', window.shortcodeglutTableAjax.nonce);
          }                                                                                                                                             
      },          
      success: function(response) {
          console.log('Shortcodeglut: AJAX response', response);

          if (!response.error) {
              // Trigger WooCommerce events
              $(document.body).trigger('added_to_cart', [response.fragments, response.cart_hash, $button]);

              // Small delay to show the loading effect
              setTimeout(function() {
                  transformToViewCart($button, cartUrl);                                                                                                
              }, 600);
          } else {                                                                                                                                      
              console.error('Shortcodeglut: Error adding to cart', response);
              // On error, reset the button
              resetButton($button);
              // Show error message
              alert('Error: ' + (response.message || 'Could not add product to cart'));
          }
      },
      error: function(xhr, status, error) {
          console.error('Shortcodeglut: AJAX error', {
              status: status,
              error: error,
              response: xhr.responseText
          });
          // On error, reset the button
          resetButton($button);                                                                                                                         
      }
  });  
              }
          }
      });

      // Show loading state on button                                                                                                                   
      function showLoadingState($button) {
          // Store original button content                                                                                                              
          var originalHtml = $button.html();
          $button.data('original-html', originalHtml);

          // Add loading class
          $button.addClass('shortcodeglut-add-to-cart-loading');

          // Fade out current content
          $button.css('opacity', '0.8');

          // Show loading spinner with SVG (white color)
            $button.html('<i class="fa-solid fa-spinner fa-spin"></i> <span>Adding...</span>');
        }                                                                                                                                                 
                  
      // Reset button to original state
      function resetButton($button) {
          $button.removeClass('shortcodeglut-add-to-cart-loading');
          var originalHtml = $button.data('original-html');
          if (originalHtml) {
              $button.html(originalHtml);
          }                                                                                                                                             
          $button.css('opacity', '1');
      }                                                                                                                                                 
                  
      // Transform button to "View Cart"
      function transformToViewCart($button, cartUrl) {
          // Prevent double transformation
          if ($button.hasClass('shortcodeglut-view-cart')) {
              return;
          }

          // Remove loading class
          $button.removeClass('shortcodeglut-add-to-cart-loading');

          // Fade out effect before transformation
          $button.css({
              'opacity': '0.4',
              'transition': 'opacity 0.2s ease-out'
          });

          // Update button appearance after fade                                                                                                        
          setTimeout(function() {
              $button.removeClass('ajax_add_to_cart add_to_cart_button added')                                                                          
                        .addClass('shortcodeglut-view-cart')
                        .attr('href', cartUrl);

              // Use SVG icon - cart with eye (view cart)

            $button.html('<i class="fa-solid fa-eye"></i> <span>View Cart</span>');

              // Fade in the new content
              $button.css({
                  'opacity': '1',
                  'transition': 'opacity 0.3s ease-in'
              });

              // Add click handler for new tab
              $button.off('click').on('click', function(e) {
                  e.preventDefault();                                                                                                                   
                  window.open(cartUrl, '_blank');
                  return false;                                                                                                                         
              }); 
          }, 200);
      }

      // Listen for WooCommerce AJAX added_to_cart event
      $(document.body).on('added_to_cart', function(e, fragments, cart_hash, $thisButton) {
          if ($thisButton && $thisButton.hasClass('shortcodeglut-add-to-cart-btn') &&
              $thisButton.closest('.shortcodeglut-table-list-content').length) {
              var actualCartUrl = $thisButton.data('cart-url');
              if (!actualCartUrl) {
                  actualCartUrl = (typeof wc_add_to_cart_params !== 'undefined')
                      ? (wc_add_to_cart_params.cart_url || wc_add_to_cart_params.wc_cart_url || '/cart/')                                               
                      : '/cart/';
              }                                                                                                                                         
              transformToViewCart($thisButton, actualCartUrl);
          }
      });

      // Hide WooCommerce loaders in table list
      function hideWooCommerceLoaders() {
          $('.shortcodeglut-table-list-content .blockUI, .shortcodeglut-table-list-content .blockOverlay, .shortcodeglut-table-list-content.woocommerce-loader').hide();
      }

      // Run on interval to catch any loaders
      setInterval(hideWooCommerceLoaders, 100);
                                                                                                                                                        
      // Also run on DOM changes
      var observer = new MutationObserver(function(mutations) {                                                                                         
          hideWooCommerceLoaders();
      });
      observer.observe(document.body, { childList: true, subtree: true });

      // Prevent WooCommerce from adding "View cart" link after our buttons
      $(document).ajaxSuccess(function(event, xhr, settings) {
          if (settings.url && settings.url.indexOf('wc-ajax') !== -1) {
              $('.shortcodeglut-table-list-content .shortcodeglut-view-cart').parent().find('.added_to_cart').remove();
          }                                                                                                                                             
      });
                                                                                                                                                        
  })(jQuery);     