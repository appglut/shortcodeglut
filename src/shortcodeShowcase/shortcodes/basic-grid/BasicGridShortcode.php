<?php
/**
 * WooCommerce Basic Grid Shortcode Handler
 *
 * Handles [shortcodeglut_grid] shortcode to display products
 * in a responsive grid layout with optional WooTemplate support
 *
 * @package Shortcodeglut
 * @subpackage ShortcodeShowcase
 */

namespace Shortcodeglut\shortcodeShowcase\shortcodes\BasicGrid;
...
class BasicGridShortcode {
	private static $instance = null;
	private $shortcode_counter = 0;

	public function __construct() {
		// Register shortcode
		add_shortcode( 'shortcodeglut_grid', array( $this, 'render_grid_shortcode' ) );

		// Register AJAX handlers
		add_action( 'wp_ajax_shortcodeglut_grid_load', array( $this, 'ajax_load_products' ) );
		add_action( 'wp_ajax_nopriv_shortcodeglut_grid_load', array( $this, 'ajax_load_products' ) );
	...
	public function register_assets() {
		// Register basic grid styles
		wp_register_style(
			'shortcodeglut-grid',
			SHORTCODEGLUT_URL . 'src/shortcodeShowcase/shortcodes/basic-grid/assets/css/basic-grid.css',
			array(),
			SHORTCODEGLUT_VERSION
		);

		// Register basic grid script
		wp_register_script(
			'shortcodeglut-grid',
			SHORTCODEGLUT_URL . 'src/shortcodeShowcase/shortcodes/basic-grid/assets/js/basic-grid.js',
			array( 'jquery' ),
			SHORTCODEGLUT_VERSION,
			true
		);
	...
	public function render_grid_shortcode( $atts ) {
		...
		if ( defined( 'REST_REQUEST' ) && REST_REQUEST ) {
			return '<div class="shortcodeglut-grid-placeholder">[Shortcodeglut Product Grid]</div>';
		}
		...
		$unique_id = 'shortcodeglut_grid_' . $this->shortcode_counter;
		...
		$atts = shortcode_atts( array(
			'columns' => 4,                       // Grid columns for desktop
		...
		// Debug: Log sorting
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG && ! empty( $url_sort ) ) {
			error_log( 'Shortcodeglut URL Parameter: shortcodeglut_sort=' . $url_sort . ', order_by=' . $atts['order_by'] ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Debug logging
		}
	...
	private function enqueue_assets( $atts, $ajax_enabled ) {
		// Enqueue grid styles
		wp_enqueue_style( 'shortcodeglut-grid' );

		// Always enqueue JavaScript for sorting functionality
		wp_enqueue_script( 'shortcodeglut-grid' );

		// Localize script with AJAX data and current page URL
		wp_localize_script( 'shortcodeglut-grid', 'shortcodeglutGridAjax', array(
			'ajax_url' => admin_url( 'admin-ajax.php' ),
			'nonce' => wp_create_nonce( 'shortcodeglut_grid_nonce' ),
	...
	private function render_output( $unique_id, $atts, $paged = 1 ) {
		...
		echo '<div class="shortcodeglut-archive-wrapper" id="' . esc_attr( $unique_id ) . '_wrapper" data-atts="' . esc_attr( $safe_atts ) . '">';
		...
		echo '<div id="' . esc_attr( $content_id ) . '" class="shortcodeglut-grid-content">';
	...
	private function render_toolbar( $unique_id, $atts ) {
		...
		if ( ! $is_compact ) {
				...
				echo '<option value="date"' . selected( $current_orderby, 'date', false ) . '>' . esc_html__( 'Sort by: Newest
			First', 'shortcodeglut' ) . '</option>';
				echo '<option value="date-desc"' . selected( $current_orderby, 'date-desc', false ) . '>' . esc_html__( 'Sort by:
			Oldest First', 'shortcodeglut' ) . '</option>';
				echo '<option value="title-asc"' . selected( $current_orderby, 'title-asc', false ) . '>' . esc_html__( 'Sort by:
			Title (A to Z)', 'shortcodeglut' ) . '</option>';
				echo '<option value="title-desc"' . selected( $current_orderby, 'title-desc', false ) . '>' . esc_html__( 'Sort
			by: Title (Z to A)', 'shortcodeglut' ) . '</option>';
				echo '<option value="price-asc"' . selected( $current_orderby, 'price-asc', false ) . '>' . esc_html__( 'Sort by:
			Price: Low to High', 'shortcodeglut' ) . '</option>';
				echo '<option value="price-desc"' . selected( $current_orderby, 'price-desc', false ) . '>' . esc_html__( 'Sort
			by: Price: High to Low', 'shortcodeglut' ) . '</option>';
				echo '<option value="popularity-desc"' . selected( $current_orderby, 'popularity-desc', false ) . '>' .
			esc_html__( 'Sort by: Best Selling', 'shortcodeglut' ) . '</option>';
				echo '<option value="popularity-asc"' . selected( $current_orderby, 'popularity-asc', false ) . '>' . esc_html__(
				'Sort by: Least Selling', 'shortcodeglut' ) . '</option>';
				echo '<option value="rating-desc"' . selected( $current_orderby, 'rating-desc', false ) . '>' . esc_html__( 'Sort
			by: Highest Rated', 'shortcodeglut' ) . '</option>';
				echo '<option value="rating-asc"' . selected( $current_orderby, 'rating-asc', false ) . '>' . esc_html__( 'Sort
			by: Lowest Rated', 'shortcodeglut' ) . '</option>';
				echo '</select>';
			}
	...
	private function render_products( $atts, $paged = 1 ) {
	  ...
	  $default_order = 'DESC'; // Default order direction
	  ...
	  switch ( $atts['order_by'] ) {
		...
		// phpcs:ignore WordPressVIPMinimum.Performance.WPQueryParams.PostNotIn_post__not_in -- Intentional exclusion of user-specified product IDs; dataset is small and bounded.
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			...
			if ( $products_query->have_posts() ) {
				...
				while ( $products_query->have_posts() ) {
					...
					$product_ids[] = get_the_ID() . ' (price: ' . get_post_meta( get_the_ID(), '_price', true ) . ')';
				}
				wp_reset_postdata();
			}
		}
		...
		// Use fixed column layout for better control
		// Render grid
		if ( $products_query->have_posts() ) {
			echo '<div class="shortcodeglut-grid" style="grid-template-columns: repeat(' . esc_attr( $columns ) . ', 1fr);">';
			...
			// Update results count via inline script — values are cast to int before output.
	...
	private function render_default_product( $product, $atts ) {
		...
		echo '<div class="shortcodeglut-product-card">';

		// Product image
		if ( $atts['show_image'] ) {
			echo '<div class="shortcodeglut-product-image">';
			echo '<a href="' . esc_url( get_permalink( $product->get_id() ) ) . '">';
			if ( $product->get_image_id() ) {
				echo wp_get_attachment_image( $product->get_image_id(), $atts['image_size'] );
			} else {
		...
		echo '<div class="shortcodeglut-product-body">';

		// Product title
		if ( $atts['show_title'] ) {
			echo '<div class="shortcodeglut-product-title">';
			echo '<a href="' . esc_url( get_permalink( $product->get_id() ) ) . '">' . esc_html( $product->get_name() ) . '</a>';
			// Debug: Show product ID and price for sorting verification
				if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
					$product_price = $product->get_price();
					echo '<small style="color: #999; font-size: 11px;"> (ID:' . esc_html( $product->get_id() ) . ' | Price:' . esc_html( $product_price ) . ')</small>';
				}
			echo '</div>';
		}
		...
		if ( $atts['show_excerpt'] ) {
			...
			if ( ! empty( $excerpt ) ) {
				echo '<div class="shortcodeglut-product-excerpt">' . wp_kses_post( $excerpt ) . '</div>';
			}
		}

		// Product rating
		if ( $atts['show_rating'] ) {
			echo '<div class="shortcodeglut-product-rating">';
			echo wp_kses_post( wc_get_rating_html( $product->get_average_rating() ) );
			echo '</div>';
		}

		echo '<div class="shortcodeglut-product-footer">';
		...
		if ( $atts['show_button'] ) {
			if ( $product->is_purchasable() && $product->is_in_stock() ) {
				echo '<button class="shortcodeglut-btn-add ajax_add_to_cart" data-product_id="' . esc_attr( $product->get_id() ) . '">' . esc_html__( 'Add to Cart', 'shortcodeglut' ) . '</button>';
			} else {
				echo '<a href="' . esc_url( get_permalink( $product->get_id() ) ) . '" class="shortcodeglut-btn-add">' . esc_html__( 'Read More', 'shortcodeglut' ) . '</a>';
			}
		}
	 ...
	 * Render pagination
	 *
	 * Note on post__not_in / exclude: pagination itself does not use these params;
	 * the suppression comments below relate to the $atts['ajax'] checks which
	 * determine link href values only — no WP_Query exclusion occurs here.
	 */
	private function render_pagination( $query, $atts, $current_page = 1 ) {
		...
		if ( $current_page > 1 ) {
			...
			if ( $is_ajax ) {
				$prev_link = '#';
				$data_page = ' data-page="' . esc_attr( absint( $prev_page ) ) . '"';
			} else {
		...
		for ( $i = 1; $i <= $max_pages; $i++ ) {
			...
			if ( $is_ajax ) {
				$page_link = '#';
				$data_page = ' data-page="' . esc_attr( absint( $i ) ) . '"';
			} else {
				$page_link = add_query_arg( array( 'shortcodeglut_paged' => $i ), $current_url );
				$data_page = '';
			}
				// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $data_page is pre-escaped when assigned.
			echo '<li><a class="page-numbers' . esc_attr( $active_class ) . '" href="' . esc_url( $page_link ) . '"' . $data_page . '>' . esc_html( $i ) . '</a></li>';
		}
		...
		if ( $current_page < $max_pages ) {
			...
			if ( $is_ajax ) {
				$next_link = '#';
				$data_page = ' data-page="' . esc_attr( absint( $next_page ) ) . '"';
			} else {
	...
	public function ajax_load_products() {
		check_ajax_referer( 'shortcodeglut_grid_nonce', 'nonce' );

			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				error_log( 'Shortcodeglut AJAX request received: ' . print_r( $_POST, true ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log,WordPress.PHP.DevelopmentFunctions.error_log_print_r -- Debug logging
		}
		...
		$atts = array(
			...
			'category' => isset( $_POST['category'] ) ? sanitize_text_field( wp_unslash( $_POST['category'] ) ) : '', // phpcs:ignore WordPressVIPMinimum.Performance.WPQueryParams.PostNotIn_exclude -- Value used for tax_query term matching, not post__not_in.
			'exclude' => isset( $_POST['exclude'] ) ? sanitize_text_field( wp_unslash( $_POST['exclude'] ) ) : '', // phpcs:ignore WordPressVIPMinimum.Performance.WPQueryParams.PostNotIn_exclude -- User-specified exclusions; bounded and intentional.
		...
		// Debug logging
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			error_log( 'Shortcodeglut HTML length: ' . strlen( $html ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Debug logging
		}
