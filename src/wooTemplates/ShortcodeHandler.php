...
class ShortcodeHandler {
    ...
    public function renderTemplateShortcode($atts) {
        ...
        $css = wp_strip_all_tags($template['template_css']);

        // Generate unique ID for this template instance
        $template_instance_id = 'shopglut-template-' . $template['id'] . '-' . uniqid();

        // Enqueue the base style and add inline CSS
        wp_enqueue_style('shopglut-woo-templates');
        if (!empty($css)) {
            // Use a unique handle for each template's inline CSS
            $inline_handle = 'shopglut-woo-template-' . $template_instance_id;
            wp_register_style($inline_handle, false, array(), defined('SHORTCODEGLUT_VERSION') ? SHORTCODEGLUT_VERSION : '1.0.0');
        ...
        return sprintf(
            '<div id="%s" class="shopglut-template">%s</div>',
            esc_attr( $template_instance_id ),
            wp_kses_post( $html ) // Escape HTML output
        );
    }
