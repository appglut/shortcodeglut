...
class WooTemplatesListTable extends \WP_List_Table {
    ...
    public function column_template_name($item) {
        ...
        // Prepare edit URL with nonce for non-default templates
        $edit_nonce_url = '';
        if ( ! $is_default ) {
            $edit_nonce_url = wp_nonce_url(
                admin_url('admin.php?page=shortcodeglut&editor=woo_template&template_id=' . $item['id']),
                'edit_template_' . $item['id']
            );
            $actions['edit'] = sprintf(
                '<a href="%s">%s</a>',
                esc_url($edit_nonce_url),
