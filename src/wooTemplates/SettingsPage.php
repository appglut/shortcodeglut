...
class SettingsPage {
	...
	public function enqueueEditorScripts( $hook ) {
		...
		wp_enqueue_style(
			...
			SHORTCODEGLUT_URL . 'src/wooTemplates/template-editor.css',
			array(),
			SHORTCODEGLUT_VERSION
		);

		// Enqueue template editor scripts
		wp_enqueue_script(
			'shortcodeglut-template-editor',
			SHORTCODEGLUT_URL . 'src/wooTemplates/template-editor.js',
		...
		// Localize script
		wp_localize_script( 'shortcodeglut-template-editor', 'shortcodeglutTemplateEditor', array(
			'previewNonce' => wp_create_nonce( 'shopglut_preview_nonce' ),
			'listUrl' => admin_url( 'admin.php?page=' . $this->menu_slug . '&view=woo_templates' ),
			'i18n' => array(
				'confirmCancel' => esc_html__( 'Are you sure you want to cancel? Any unsaved changes will be lost.', 'shortcodeglut' ),
				'saving' => esc_html__( 'Saving...', 'shortcodeglut' ),
				'addHtmlTemplate' => esc_html__( 'Add HTML template to see the preview', 'shortcodeglut' ),
				...
				'unknownError' => esc_html__( 'Unknown error', 'shortcodeglut' ),
			)
		) );
	}
	 ...
	 * Note: Input data should already be sanitized before calling this method
	 */
	private function saveTemplate( $template_name, $template_slug, $template_html, $template_css, $template_tags, $template_id = null ) {
		global $wpdb;
		$table_name = $wpdb->prefix . 'shopglut_woo_templates';

		// Data is already sanitized by caller, validate required fields
		// Input sanitization happens in templateEditorPage() before calling this method
	...
	public function templateEditorPage() {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Nonce verification happens below when template_id is present
		$template_id = isset( $_GET['template_id'] ) ? absint( $_GET['template_id'] ) : 0;
		$template = null;

		// If template_id is provided and not 0, verify nonce and fetch the template data from the database
		if ( $template_id > 0 ) {
			// Verify nonce for editing existing template
			// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Nonce verification happens here
			if ( ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'edit_template_' . $template_id ) ) {
				wp_die( esc_html__( 'Security check failed. Invalid nonce for editing template.', 'shortcodeglut' ) );
			}
		...
		if ( isset( $_POST['save_template'] ) && isset( $_POST['template_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['template_nonce'] ) ), 'save_woo_template' ) ) {
			// Extract and validate form data - sanitized immediately
			...
			$saved_template_id = $this->saveTemplate( $template_name, $template_slug, $template_html, $template_css, $template_tags, $template_id > 0 ? $template_id : null );
			if ( $saved_template_id ) {
				// Redirect to edit page with template_id, nonce, and success message
				$redirect_url = wp_nonce_url(
					admin_url( 'admin.php?page=' . $this->menu_slug . '&editor=woo_template&template_id=' . $saved_template_id . '&saved=1' ),
					'edit_template_' . $saved_template_id
				);
				...
				<div class="scg-form-section">
					<div class="scg-form-row">
						...
						<div class="scg-form-group scg-form-half">
							...
							<small>Use lowercase letters, numbers, and underscores only. <?php echo esc_html( $template_id ? 'Cannot be changed after creation.' : 'Used in shortcode: [shopglut_template id="template-id"]' ); ?></small>
						</div>
					</div>
				</div>
...
@media (max-width: 768px) {
	...
	public function ajaxDuplicateTemplate() {
		// Check nonce for security
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'shortcodeglut_duplicate_template' ) ) {
			wp_send_json_error( array( 'message' => 'Security check failed.' ) );
			return;
		}
	...
	public function handleTemplateActions() {
		...
		// Check user permissions
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have sufficient permissions to perform this action.', 'shortcodeglut' ) );
		}

		// Handle delete action
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Nonce verification is performed below
		if ( isset( $_GET['action'] ) && 'delete' === sanitize_text_field( wp_unslash( $_GET['action'] ) ) && isset( $_GET['template_id'] ) ) {
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Nonce verification is performed below
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Nonce verification is performed below with check_admin_referer()
		if ( isset( $_POST['action'] ) && 'delete' === sanitize_text_field( wp_unslash( $_POST['action'] ) ) && isset( $_POST['template_ids'] ) && is_array( $_POST['template_ids'] ) ) {
			// Verify nonce
			check_admin_referer( 'bulk-templates' );

			$deleted = 0;
			$template_ids = array_map( 'absint', wp_unslash( $_POST['template_ids'] ) );
